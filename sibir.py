...
host = 'ftp.esphere.ru'  # хост
ftp_user = 'hko_sibir'  # логин
ftp_password = '0SMP1ER-d255Qz4-Faws'  # пароль для входа
...
login = 'admin'
user_secret = '90f819c60350983f966129f440b0bf4d0fc49136'
org_id = '2bcbe3aa-a580-11ea-80e0-d8d385655247'
urlserv = "http://91.195.178.122:7680/resto" # "http://192.168.3.200:8080/resto"   http://91.195.178.122:7680/resto
...
f.write(str(token) + '\n')
f.close()
# http://api-delivery.iiko.ru:8080/resto/api/corporation/stores/?key=
...
while True:
    ...
    if len(new_files) > 0:
        ...
        for new_file in new_files:
            ...
            #f.write('<document><dateIncoming>2020-11-04T23:59:00</dateIncoming><useDefaultDocumentTime>true</useDefaultDocumentTime><accountToCode>5.01</accountToCode><revenueAccountCode>4.01</revenueAccountCode>')
            with open('mats/' + new_file, 'wb') as localfile:
                ftp.retrbinary('RETR ' + new_file, localfile.write)
            tree = ET.parse("mats/" + new_file)
            # tree = ET.parse("mats/test.xml")
                ...
                if obj.text == str('DP'):                                       # Место доставки
                    ...
                    for mem in member.iter('C059'):
                        ...
                        if obj.text != None:
                            ...
                            adressdost = obj.text
                            #f.write('<comment>TESTTESTTEST He npoBoguTb</comment><items>')
            ...
            #organiz = organiz.replace('»', '')
            #organiz = organiz.replace('-1', '')
            #orgdost = orgdost.replace('«', '')
            #orgdost = orgdost.replace('»', '')
            #orgdost = orgdost.replace('-1', '')
            #organiz = transliterate.translit(str(organiz), reversed=True)
    #+ transliterate.translit(str(organiz), reversed=True) + ' '
            print(orgdost)
            ...
            if "РОЗНИЦА" in str(orgdost):
                ...
                #f.write(' ИНН ' + str(memeinn) + ' КПП ' + str(memekpp) + ' ' + numzakaz + '</comment><items>')
        ...
        #r = requests.get('http://192.168.3.200:8080/resto/api/logout?key=' + key)
        #print(r.text)
        #time.sleep(10)
    else:  
